...
description: 協助修改 GOGOCARE 集氣網站（power-cheer，積分衝刺月集氣計畫）的前端頁面 —— 文案、課程清單與封面圖、顏色樣式、折扣門檻等常見異動。當使用者提到「集氣網頁」「集氣活動頁」「積分衝刺月」「0703 那個頁面」，或要求調整 power-cheer/frontend、power-cheer/backend/Config.gs 內容時使用。使用者通常不會寫程式，請用他聽得懂的方式確認需求，實際修改都由你（Claude）動手完成。
---
...
```
使用者瀏覽器
  → power-cheer/frontend/0703.html + style.css + app.js + api.js   ← 部署在 GitHub Pages
  → 呼叫 Google Apps Script Web App（power-cheer/backend/*.gs）    ← 用 clasp 推送
  → 讀寫 Google Sheet（PowerLog / Summary）
```

**兩條完全不同的部署管道**：
- 改 `frontend/` 底下的檔案 → 用 **git push** 上傳到 GitHub，GitHub Pages 會自動更新網頁。
...
| 使用者想改的東西 | 要改的檔案 |
| --- | --- |
| 頁面文字、標題、活動說明、期間 | `frontend/0703.html`（畫面結構/預設文字）＋ `backend/Config.gs` 的 `EVENT_PURPOSE` / `EVENT_PERIOD`（頁面載入後會被這個蓋過去） |
...
| 折扣門檻（滿多少人打幾折） | **同時**改 `backend/Config.gs` 的 `CONFIG.DISCOUNT_TIERS` 和 `frontend/app.js` 開頭的 `DISCOUNT_TIERS`（兩份要一模一樣，見下方「重要」） |
| 顏色、字體大小、卡片樣式、版面 | `frontend/style.css`（顏色變數集中在檔案最上面的 `:root`，對照 `docs/design.md`） |
...
1. **折扣門檻 `DISCOUNT_TIERS`**：`backend/Config.gs` 和 `frontend/app.js` 各存了一份一模一樣的陣列（前端用來畫進度條，屬於暫時性的重複設計）。改一邊沒改另一邊，會出現「頁面顯示的折扣」跟「後端實際判定的折扣」對不上的 bug。**只要改門檻，兩個檔案都要改。**
2. **課程資料只在後端**：`frontend/0703.html` 沒有寫死任何課程名稱或圖片，都是頁面載入時向後端 API 要資料再畫出來的。所以新增/修改課程只需要改 `backend/Config.gs`，改完要 `clasp push` 才會反映到頁面上（純改 frontend 檔案不會生效）。
...
2. 若要連預設文字（API 打不到時的備援畫面）一起改，同步修改 `frontend/0703.html` 裡 `id="eventPurpose"` / `id="eventPeriod"` 的段落。

**改顏色 / 風格**
1. 打開 `frontend/style.css`，顏色都定義成 CSS 變數在檔案最上面（`--color-primary` 等），對照 `docs/design.md` 的色票表。
...
1. 有 Node.js 環境的話：`cd power-cheer && npm test`，確認 48+ 個自動化測試都還過（涵蓋驗證規則、24 小時限制、折扣門檻計算等邏輯）。
2. 沒有 Node.js 也至少對改過的 `.js` 檔跑語法檢查：`node --check frontend/app.js`。
...
**只改了 `frontend/` 裡的檔案**：
```bash
git add power-cheer/frontend
git commit -m "說明改了什麼"
git push
```
GitHub Pages 通常 1 分鐘內會更新，使用者若還看到舊畫面，請他按 Ctrl+F5（強制重新整理）。目前實際能打開的網址是
`https://service-gogocare.github.io/form/power-cheer/frontend/0703.html`（注意檔名是 `0703.html`，不是 `index.html`）。
